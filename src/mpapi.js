...
export class mpapi {
  constructor(serverUrl, identifier) {
    ...
    console.log(
      "mpapi initialized with serverUrl:",
      this.serverUrl,
      "and identifier:",
      this.identifier
    );
  ...
  _connect() {
    ...
    this.socket.addEventListener("message", (event) => {
      this.stats.rx.tick(
        1,
        event.data.byteLength ? event.data.byteLength : event.data.length
      );
      ...
      if (cmd === "host") {
        ...
        if (this.onHost) this.onHost(session, clientId, data);
      } else if (cmd === "join") {
        /*
					session: sessionId,
					name: session.name,
        ...
        if (this.onJoin) this.onJoin(payload);
      } else if (cmd === "list") {
        if (this.onList) this.onList(data);
      } else if (
        cmd === "joined" ||
        cmd === "left" ||
        cmd === "closed" ||
  ...
  _enqueueOrSend(serializedMessage) {
    this.stats.tx.tick(
      1,
      serializedMessage.byteLength ? serializedMessage.length : 0
    );
  ...
  host(data = {}) {
    return new Promise((resolve, reject) => {
      if (this.sessionId) return reject("error_already_hosting_or_joined");
